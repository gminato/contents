...
import { AppMetadata, App } from '../app.component';
...
export class ManagementFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.chosenContent = 'application';
  }

  openDialog(types: string) {
    let dialogRef;
    if (this.chosenContent.toLowerCase() === 'application') {
      dialogRef = this.dialog.open(SoftwareManagementFormComponent);
    } else if (this.chosenContent.toLowerCase() === 'tunes') {
...
export class SoftwareManagementFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.appForm = this.formBuilder.group({
      ...
      website: [''],
      thumbnail: [''],
      screenshotUrls: this.formBuilder.array([]), // Array of strings
      rating: [null, [Validators.min(0), Validators.max(5)]],
      description: [''],
      developerEmail: [''],
      shortDescription: [''],
      version: [''],
      fileLink: ['', Validators.required, this.validateUrl],
      createdby: [''],
      lastupdatedby: [''],
      appBanner: [''],
      packageName: [''],
    ...
    this.appForm.valueChanges.subscribe((changes) => console.log(changes));
  }

  ngOnInit() {
    // Add an initial screenshot URL field
    this.addScreenshotUrl();
  }
  ...
  get screenshotUrlsArray() {
    return this.appForm.get('screenshotUrls') as FormArray;
  }

  // Function to add a screenshot URL control
  addScreenshotUrl() {
    const screenshotUrlsArray = this.screenshotUrlsArray;
    screenshotUrlsArray.push(this.formBuilder.control(''));
  }

  // Function to remove a screenshot URL control by index
  removeScreenshotUrl(index: number) {
    const screenshotUrlsArray = this.screenshotUrlsArray;
    if (index >= 0 && index < screenshotUrlsArray.length) {
      screenshotUrlsArray.removeAt(index);
    }
  }
  ...
  onSubmit() {
    if (this.appForm.valid) {
      ...
      // You can access form values using this.appForm.value
      // For example, to get the 'name' field value:
      // const name = this.appForm.get('name').value;
    } else {
